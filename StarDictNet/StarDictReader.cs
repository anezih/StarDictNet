...
public class StarDictReader
{
    ...
    public StarDictReader(string ifoPath)
    {
        try
        {
    ...
    public StarDictReader(Stream ifoStream, Stream idxStream, Stream synStream, Stream dictStream, CompressionOptions compressionOptions)
    {
    ...
    public StarDictReader(Stream ifoStream, Stream idxStream, Stream dictStream, CompressionOptions compressionOptions)
    {
        Metadata = new Ifo(ifoStream);
        Idx = new IdxReader(idxStream, (Metadata.IdxOffsetBits == 64), Metadata.IdxFileSize, compressionOptions.IDX).IdxData;
        if (compressionOptions.DICT)
