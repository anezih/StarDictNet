...
public class DictZip
{
    ...
    private byte[] GetByteArrayChunk(Stream stream, int offset, int chunkSize)
    {
    ...
    private string GetZeroTerminatedString(Stream stream, int offset)
    {
        var chunk = GetByteArrayChunk(stream, offset, 1024);
    ...
    {
        int pos = 0;
        var first10 = GetByteArrayChunk(DictZipStream, 0, 10);
        ...
        {
            var _fextra = GetByteArrayChunk(DictZipStream, pos, 2);
            header.FEXTRA.XLEN = BitConverter.ToUInt16(_fextra);
            pos += 2;
        }

        var fextraSubfields = GetByteArrayChunk(DictZipStream, pos, header.FEXTRA.XLEN);
        ...
        {
            header.FNAME = GetZeroTerminatedString(DictZipStream, pos);
            pos += header.FNAME.Length + 1;
        }

        if ((header.FLG & FCOMMENT) != 0x00)
        {
            header.FCOMMENT = GetZeroTerminatedString(DictZipStream, pos);
            pos += header.FCOMMENT.Length;
        }

        if ((header.FLG & FHCRC) != 0x00)
        {
            var fextra = GetByteArrayChunk(DictZipStream, pos, 2);
